...
func CreateJob(job models.Job, userId int) (models.Job, error) {
	...
	err := utils.DB.QueryRow(
		`INSERT INTO job(job_title, company, location, salary, url, user_id ) VALUES($1, $2, $3, $4, $5, $6) RETURNING id`,
		job.JobTitle, job.Company, job.Location, job.Salary, job.URL, userId,
	).Scan(&id)
...
func UpdateJob(job models.Job) error {
	fmt.Print(job.ID, "this is ghe job id")
